...
namespace RosterLib
{
   ...
   public class NFLPlayer : IComparable
   {
      ...
      {
         LastSeason = "";
         Utility.Announce("2. Instantiating new Player " + nameIn);
      ...
      public bool IsPlayerReport()
      {
         ...
         if ( exists ) 
            Utility.Announce( string.Format( "A player report for {0} already exists", PlayerName ) );
      ...
      public bool IsAtHome(NFLWeek week)
      {
         ...
         if (game == null)
            Utility.Announce(string.Format("  no game found for {0} in week {1}", TeamCode, week.WeekKey()));
         else
      ...
      public void TallyProjections(bool allGames, bool currSeasonOnly, string whichSeason)
      {
         ...
         {
#if DEBUG
            Utility.Announce(string.Format("NFLPlayer.TallyProjections for {0}", PlayerName));
#endif
            //TODO: implement
         }
#if DEBUG
         Utility.Announce(string.Format("NFLPlayer.TallyPerformance for {0} done", PlayerName));		
#endif
      }
      ...
      public void AddMetric( string metricType, string gameKey, decimal nScores, int fp )
      {
         ...
         var pgm = GameMetrics.ContainsKey( gameKey ) 
                                    ...
                                    new PlayerGameMetrics
                                       {
                                          GameKey = gameKey, PlayerId = PlayerCode
                                       };
